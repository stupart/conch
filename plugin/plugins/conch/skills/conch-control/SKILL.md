...
- **Show the user something worth looking at** — Call `review_to_front {summary, link?, session?}` ONLY when there's an actual deliverable to *look at* — a page, a diff, a rendered result, a build — and seeing it matters. It brings that work to the front of the user's screen and opens `link` (a URL or file path). This is NOT for routine "I finished": conch already announces finished turns, so use it sparingly — only when the user should stop and look. `session` names the worker whose work it is; a session surfacing its OWN finished turn should end its reply with the `conch:review <summary> | <link>` marker instead (its Stop hook would otherwise overwrite a self-issued tool review).
