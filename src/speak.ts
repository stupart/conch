...
export async function trySynth(
  ...
  try {
    ...
    return { kind: "transport-failure", error: errorText(error) };
  }
  ...
  try {
    ...
    if (wasCancelled(control, options.signal)) return { kind: "cancelled" };
    health.recordReachable(token);
    return { kind: "post-header-inference-failure", status: response.status, detail: errorText(error) };
  }
}
...
async function synthPiece(
  ...
  for (const speed of speeds) {
    ...
    while (!control.cancelled) {
      last = await trySynth(cfg, piece, effectiveVoice, control, { speed });
      if (last.kind === "audio") {
  ...
  const left = await synthPiece(cfg, halves[0], effectiveVoice, control);
  const right = await synthPiece(cfg, halves[1], effectiveVoice, control);
