...
The app polls `/tmp/conch-sessions.json` for state and sends talk/stop, recite, and auto/manual controls to the running daemon over `/tmp/conch.sock`. The daemon still owns microphone capture, transcription, and speech synthesis, so start conch before opening the app.
