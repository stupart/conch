...
| B5 | ~~**Approvals** (the four-way decision)~~ — **done** (2026-09-11), by voice. A `permission_prompt` on a session started with `bypass-permissions` off announces the tool and what it wants — the newest `tool_use` with no `tool_result`, read from the transcript, because the notification carries only "Claude needs your permission to use Bash" and fires for an AskUserQuestion too (skipped) — opens the mic, and presses what a person would press: "yes" is Enter on the highlighted row, "no" is Escape, "no, use main instead" is Escape and the alternative typed as the next prompt, and "always" walks Down to the don't-ask-again row after a second spoken yes, the one outcome that outlives the prompt and is delivered blind through a menu conch cannot see. Unclear is re-asked once, then left for the keyboard with the row still saying `permission: Bash — git push`. With `bypass-permissions` on (Tyler's default) nothing changes. Not yet verified on a live dialog — the keys are Claude Code's interactive bindings, not something a test drives — so the ten seconds still apply to that. The protocol-grade upgrade is Claude Code's `PermissionRequest` hook (`tool_name`, `tool_input` in; `allow`/`deny` out; 600 s timeout, dialog stays up meanwhile): it needs the hook to block on the daemon socket for a reply, which the control server does not do. Codex: nothing reaches conch — its approval prompt is neither hooked nor on the wire (560 of 561 local rollouts ran `approval_policy: never`, none records an approval event), and `--ask-for-approval on-request\|never` is the whole knob. **Checkpoint/revert** — still open. Claude Code exposes `/rewind` (Esc Esc on an empty prompt): restore code, conversation, or both, plus summarize-from-here — with no hook, SDK, or CLI way to list or trigger a checkpoint, so driving it means keystrokes into a picker where being wrong destroys work. Codex has no checkpoint to drive. Blocked on a programmatic rewind that neither agent exposes, and on the same ten seconds. | V |
