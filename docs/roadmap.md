...
| | add | lands in |
|---|---|---|
| C1 | ~~**Configuration at session START.**~~ — **done** (2026-09-11): the affordances of each agent's own CLI, read from `--help` on this Mac and mirrored as a per-agent table on the adapter row (`startOptions` in `agent-adapter.ts`: name, flag, kind, choices, the CLI's own help line). **Claude Code** (2.1.266): `--model` (text), `--permission-mode` (acceptEdits · auto · bypassPermissions · manual · dontAsk · plan), `--dangerously-skip-permissions` (toggle), `--effort` (low · medium · high · xhigh · max), `--fork-session` (toggle, resume only). **Codex** (0.153.4): `--model` (text), `--sandbox` (read-only · workspace-write · danger-full-access), `--ask-for-approval` (on-request · never), `--dangerously-bypass-approvals-and-sandbox` (toggle), `--profile` (text). The `session-start` request carries `options: Record<name, string|boolean>`; the socket validator and every direct launcher refuse an unknown or malformed option quoting the CLI's help, and the launch command renders the accepted ones after the resume/teleport argument, shell-quoted. The Mac sheet, the phone's sheet and `conch start` show exactly the table for the chosen agent — a toggle, a segmented control (a menu past three choices), a field — with the `bypass-permissions` setting as the toggle's starting value; the request's toggle then wins. **Deliberately left out:** `--allowedTools` / `--disallowedTools` / `--tools`, `--add-dir` and Codex's `-c key=value` (free-form, nothing to validate against); `--continue` (the resume picker covers it); `-n/--name` (`conch rename` covers it); reasoning effort on Codex (not a flag in this build, only a config key); Codex's `--full-auto` (absent from 0.153.4), `--approve-for-me` and `--search` (not asked for). The TUI's start overlay still sends no options. The only moment a toggle is honest. Turns B3's hard problem into a non-problem. | C |
