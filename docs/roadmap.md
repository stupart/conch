...
| C5 | ~~**Make the plugin genuinely useful**, and review it in detail~~ — **done (2026-09-11)**, as the reviewed contract. **What changed:** `conch_config` now changes only a named allowlist (`AGENT_TUNABLE_SETTINGS`: `end-silence`, `voice-speed`, `haiku-timeout`, `read-full`, `announce-summary`, `whisper-idle-unload`) and refuses every other key with the `conch set` command the user can run; `conch_speak` refuses text over 600 characters (never cuts it) and a second call while one is pending; `conch_mode` switches only the calling session unless `scope: "all"` is passed explicitly, through the same scoped event the Mac's per-row control sends; `conch_wake`/`conch_recite` results say where the audio went, from the published `audioControl`; the contract doc names the allowlist, lists every refusal, covers two Macs, the phone and `conch help-session`, and no longer says "curated". **Deliberately not added:** no tool to start, close, or inject into another session (C2, held on purpose); no tool touching plugins, MCP servers, or permissions (B3 is the user's pass); no `conch_config` access to topology or security keys, by name, ever; and the review's `speak-sentences`, `tts-voices`, `voice`, `bell` and `mic-cues` are not in the allowlist because no such settings exist in `src/settings.ts`. | plugin |
