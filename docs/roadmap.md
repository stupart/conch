...
| A2 | **The Mac app does not respawn a dead daemon**, and hides the start toggle when it happens. `daemon-identity.ts` is written and tested; wiring is the other half. — **The respawn half is fixed** (2026-09-10, after the app sat on a dead socket twice in one day): an adopted daemon's socket is polled every 3 s and the app starts its own when it stops answering. The toggle wording and the `daemon-identity.ts` wiring remain. | C |
...
| A7 | **Duplicate terminal mouse-up?** Repeated `copied N chars` with nobody selecting. **Looked at 2026-09-10, not solved.** Ninety copies that day, always the same three sizes in the same order (94, 5, 709), while Tyler was away. Facts: the line is written by `copySelection` in the theater, which only runs on a mouse-up after a real drag (`TheaterSelection.end` clears a click without movement), and pointer events come only from a TTY stdin — so the writer is a `conch` TUI in a terminal, never the app-spawned daemon, which has no TTY. Each copy also runs `pbcopy`, so whatever this is clobbers the clipboard every time. The log line now carries a timestamp and the writer's pid; the next occurrence can be matched to what happened. Suspects: something replaying the same three drags into a theater pane (tmux, a terminal restore after sleep, or a raised window). | UI |
