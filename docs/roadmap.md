...
| C14 | **A pasted image path stays a path.** The composer already renders a thumbnail for a real attachment and falls back to a paperclip chip when the file will not decode (`ComposerView.swift:533`), so the gap is narrower than it looks: pasting the PATH of an image is text, and nothing promotes it. macOS screenshot workflows produce exactly that — a path on the clipboard — so this is the common case, not an edge one. Promote a pasted path that resolves to a decodable image into an attachment, and paste image DATA directly as one too. Worth checking the phone before building: it may only look different because it never receives paths. | UI |
