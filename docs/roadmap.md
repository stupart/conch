...
| C15 | ~~**A session started by another session, grouped under its starter.**~~ — **done** (2026-09-11). Tyler: "sometimes i have claude code start codex sessions and vice versa — probably want to group or show they're in communication somehow." What C4 already does: a LIVE background subagent nests under its interactive Claude parent. What it does not: this Mac's own conch session is a `kind: "bg"` job (`72858.json`, forked from the interactive `61637` through `claude daemon run --spawned-by`), which `isEngageable` drops, so it has no row and its subagents never nest — right, because there is no terminal to type into or announce for, and a row you cannot address would only invite both. **The new part, from the process tree only:** a Bash-tool shell inside a Claude session has the session's registry pid as its parent (measured: shell 45258 → 72858), so a `codex` it starts, or a `claude` that Codex's shell starts, carries the starter's pid in its ancestor chain. `registrySnapshot` reads one `ps -Ao pid=,ppid=` (~30 ms, only when two rows carry pids) and `withStartedBy` walks each row's chain to the nearest pid another row owns — Claude's `<pid>.json`, Codex's hook registry, or an observed thread's lock holder — and sets `startedBySessionId`. Never from labels or timing. Before this, such a pair were two unrelated rows. The row model places the started session under its starter (after the starter's subagents, in status order among siblings; a chain nests each under the one above; one whose starter is dismissed stays at the top — it is a session); the wire carries `startedBySessionId`; the Mac sidebar indents it with a "started by <label>" line beside the agent badge; both TUI renderers indent it with `↳`. Announcements, mic, composer, close and reveal are untouched: a started session is a full session. **Not covered:** a Codex thread whose writer lock nobody holds has no pid, so it can be neither starter nor started; a Codex-started `claude -p` is headless and never a row anyway; and "in communication" beyond ancestry — one session messaging another — is C2's, on hold. | C + UI |
