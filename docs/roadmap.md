...
| C8 | **Sessions on another machine.** Today the resume list reads local files only (`~/.claude/projects/**.jsonl`, Codex's thread DBs) and resuming shells out to `cd <cwd> && exec claude --resume <id>` on THIS Mac, so nothing crosses machines: a session is its transcript file plus a working directory that has to exist at the same path. Moving them is a sync problem conch should not own. Driving them is not — the relay, the pairing and the E2E protocol already exist and already carry the phone, so a second Mac as a relay client is the same shape, with the sessions staying on the machine that has the code. | C |
