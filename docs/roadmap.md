...
| A7 | **A session id is not a window, and conch assumes it is.** `claude --resume <id>` in a second terminal keeps the id, so Tyler works in two windows (`~/arch-website` and `~/arch-swap`) that share `4eb30ede` and one transcript, on separate parentUuid branches. The registry lists both and the ledger now shows both — but `panelSessions` and `sessionStates` key on the id, so both rows read one window's state, and the Mac app drops the duplicate outright (`StateStore.swift:807`). Deeper: Claude Code's hooks report only `session_id`, so conch cannot tell which window finished a turn — the hook would have to report its own window (its parent pid). Replies already route by `event.pid` and are correct; anything addressed *to* a row is not. Rendering a second row before the state behind it is per-window would just show the same state twice. Belongs with the `session-registry.ts` split. | C |
