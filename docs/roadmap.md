...
| B3 | ~~**The write pass**~~ — **done** (2026-09-11), narrowed to what the agents themselves switch: enable or disable an EXISTING plugin or MCP server for the NEXT session, at user or project scope, in the agent's own file. Verified against Claude Code 2.1.266 and Codex 0.153.4: Claude's `enabledPlugins["name@marketplace"]` in `~/.claude/settings.json` / `<project>/.claude/settings.json` (`claude plugin disable --scope user\|project`); Claude's MCP switch is per PROJECT only — its `/mcp` toggle writes `projects["<dir>"].disabledMcpServers` in `~/.claude.json`, and a `.mcp.json` server is approved or rejected through `enabledMcpjsonServers` / `disabledMcpjsonServers` there — so user scope for a Claude server is refused in words; Codex's `[plugins."id"] enabled` and `[mcp_servers.name] enabled` in `~/.codex/config.toml` / `<project>/.codex/config.toml` (`codex mcp list` shows the result as disabled). `src/config-write.ts` plans first and answers with the unified diff (`config-toggle … preview`, `conch config-toggle --preview`, and the Mac inspector's per-row toggle, which opens the diff with the scope and Apply/Cancel); only an apply writes — temp file and rename under Claude's own `<file>.lock`, `<file>.conch-backup-<ms>` kept to the newest three (`config-rollback` / `conch config-rollback` puts one back), then a readback that refuses and restores the previous bytes if the file does not parse or lacks the change; an apply carrying the preview's hash is refused when the file moved in between. TOML is an exact-line edit under the one table header, re-parsed to prove nothing else moved; JSON is re-emitted at the file's own indentation, byte-identical for files the agents wrote. **Not writable:** skills (Claude's `skillOverrides`, Codex's `skills.config`), per-tool permissions, a plugin's own MCP servers (switch the plugin), `local` scope, managed and admin layers, adding or removing anything — and nothing here reaches a running session, which every surface says. | C |
