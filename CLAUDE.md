...
- No runtime npm deps; everything shells out to macOS binaries (`say`, `afplay`, `sox`, `tmux`, `osascript`) and whisper-cli. Keep it that way. (One sanctioned exception: the intent router's api mode calls the Anthropic API via raw `fetch` — still no SDK.)
