...
test("the phone question card goes dead on a row with no terminal and says why", () => {
  ...
  const row = sliceFrom(phone, "private func questionRow(", "private func toggleSelection(");
  ...
  expect(session).toContain("guard !label.isEmpty, !optionReplyInFlight, row?.noTerminal == nil else { return }");
});
